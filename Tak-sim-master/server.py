...
class Server:
    ...
    def setLogFile(self, filename):
        self.log_file_handle = open(filename, 'w')  # Updated to open in text mode
    ...
    def RecvDataFromClient(self, client_id):
        ...
        if client_id < len(self.communicator_list):
            ...
            if data is None:
                print(f'ERROR : TIMEOUT ON CLIENT NETWORK {client_id} END')
    ...
    def SendData2Client(self, client_id, data):
        ...
        if client_id < len(self.communicator_list):
            ...
            if not success_flag:
                print(f'ERROR : COULD NOT SEND DATA TO CLIENT {client_id}')
                self.CloseClient(client_id)
    ...
    def CloseAllClients(self):
        ...
        Returns:
            None
        """
        for idx in range(len(self.communicator_list)):  # Changed from xrange to range
    ...
    def playTak(self, n, timelimit, client_0, client_1):
        ...
        if (client_0 < len(self.communicator_list)) and (client_1 < len(self.communicator_list)):
            ...
            while True:
                ...
                if not data:
                    break
                print(f'{data} Received from client 0')
                data = json.loads(data)
                ...
                print(f'{data} Received from client 1')
