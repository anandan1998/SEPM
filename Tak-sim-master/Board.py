...
class Board(object):
	...
	def draw_board_labels(self):
		...
		for i in range(self.n):  # Updated from xrange to range
			y = Board.BOTTOM_LABEL_Y + i * Board.SQUARE_SIZE
			self.canvas.create_text((x, y), text=str(self.n - i), font=("Arial", self.FONT_SIZE), fill="#ecf0f1")
		y = Board.SIDE_LABEL_Y
		for i in range(self.n):  # Updated from xrange to range
			x = Board.SIDE_LABEL_X + i * Board.SQUARE_SIZE
			self.canvas.create_text((x, y), text=chr(i + 97), font=("Arial", self.FONT_SIZE), fill="#ecf0f1")

	def draw_tiles(self, board_list):
		for r in range(self.n):  # Updated from xrange to range
			for c in range(self.n):  # Updated from xrange to range
		...
		for r in range(self.n):  # Updated from xrange to range
			for c in range(self.n):  # Updated from xrange to range
				...
				if r % 2 == c % 2:
					...
					fill = "#C2DDF6"
				self.canvas.create_rectangle(x, y, x + Board.SQUARE_SIZE, y + Board.SQUARE_SIZE, fill=fill, outline=fill)
